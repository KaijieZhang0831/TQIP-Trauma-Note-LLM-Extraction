...
# Beam/DVTS sampling controls
# - N_SAMPLES: used by Best-of-N (vote sampling)
# - NODE_SAMPLES: number of candidate steps proposed per node expansion in tree search
# - BEAM_WIDTH: number of concurrent beams/subtrees
NODE_SAMPLES = int(os.environ.get("NODE_SAMPLES", "2"))
...
def _dvts_key(trace, depth: int):
    # Use the first `depth` decisions as the subtree id.
    key = []
    for step in trace[:depth]:
        key.append(f"{step.get('node_id','')}={str(step.get('option','')).lower()}")
    return tuple(key)


def _verifier_pick_indices(task_prompt: str, candidates: list, k: int):
    # Pick up to k candidates using the verifier, without requiring a full ranking.
    ...
    while remaining and len(picked) < k:
        ...
        for j, idx in enumerate(remaining):
            candidates_lines.append(f"{j}: {json.dumps(candidates[idx], ensure_ascii=False)}")
        verifier_prompt = VERIFIER_PROMPT.format(
        ...
        vobj = extract_json(raw)
        best_local = 0
        if isinstance(vobj, dict) and isinstance(vobj.get("best_index"), int):
            bi = vobj["best_index"]
            if 0 <= bi < len(remaining):
                best_local = bi
        chosen_global = remaining.pop(best_local)
        picked.append(chosen_global)
    return picked


def _sample_candidates(prompt_text: str, n_samples: int):
    # Sample multiple candidate JSON answers for a single node prompt.
    cands = []
    structured_ct = 0
    fallback_ct = 0
    for _ in range(int(n_samples)):
        obj, used_structured, _ = _llm_router.generate_json(prompt_text)
        if used_structured:
            structured_ct += 1
        else:
            fallback_ct += 1
        if isinstance(obj, dict) and "option" in obj and "rationale" in obj:
            ...
            if opt in ("yes", "no"):
                cands.append(obj)
    return cands, structured_ct, fallback_ct


def _prune_states(states: list, max_states: int, per_key: int, key_depth: int):
    # Keep diversity by first selecting up to `per_key` states per subtree key.
    max_states = max(1, int(max_states))
    per_key = max(1, int(per_key))
    key_depth = max(1, int(key_depth))

    groups = {}
    for s in states:
        key = _dvts_key(s.get("trace", []), key_depth)
        groups.setdefault(key, []).append(s)

    selected = []
    leftovers = []
    for key, lst in groups.items():
        lst.sort(key=lambda x: x.get("score", float("-inf")), reverse=True)
        selected.extend(lst[:per_key])
        leftovers.extend(lst[per_key:])

    selected.sort(key=lambda x: x.get("score", float("-inf")), reverse=True)
    if len(selected) >= max_states:
        return selected[:max_states]

    leftovers.sort(key=lambda x: x.get("score", float("-inf")), reverse=True)
    need = max_states - len(selected)
    selected.extend(leftovers[:need])
    return selected


def run_dvts(question_key: str, context_text: str, atomic_tree: dict):
    """
    Diverse Verifier Tree Search (DVTS) over an atomic decision tree:
    - Each state expands to up to DVTS_BRANCH candidates using the verifier to pick steps.
    - States are pruned into diverse subtrees by the first DVTS_KEY_DEPTH decisions.
    """
    dvts_trees = int(os.environ.get("DVTS_TREES", str(BEAM_WIDTH)))
    dvts_branch = int(os.environ.get("DVTS_BRANCH", "2"))
    dvts_key_depth = int(os.environ.get("DVTS_KEY_DEPTH", "2"))
    dvts_per_key = int(os.environ.get("DVTS_PER_KEY", "1"))
    max_steps = int(os.environ.get("DVTS_MAX_STEPS", "200"))

    start_node = atomic_tree.get("__start__", "")
    nodes = atomic_tree.get("nodes", {})
    if not start_node or start_node not in nodes:
        return {"rationale": "Invalid atomic tree", "option": "no", "votes": {"rule": "dvts"}}

    states = [{"node_id": start_node, "score": 0.0, "trace": []}]
    total_structured = 0
    total_fallback = 0
    total_samples = 0
    total_parsed = 0
    ...
    for _step in range(max_steps):
        # Stop if all states are terminal.
        if all(str(s.get("node_id", "")).startswith("FINAL:") for s in states):
            break

        expanded = []
        for s in states:
            node_id = s.get("node_id", "")
            if str(node_id).startswith("FINAL:"):
                expanded.append(s)
                continue

            node_entry = nodes.get(node_id)
            if not (isinstance(node_entry, list) and len(node_entry) == 2 and isinstance(node_entry[0], dict) and isinstance(node_entry[1], str)):
                expanded.append({"node_id": "FINAL:no", "score": s.get("score", 0.0), "trace": s.get("trace", []) + [{"node_id": node_id, "option": "no", "rationale": "Invalid node entry"}]})
                continue

            trans = node_entry[0]
            node_body = fill_medical_notes(node_entry[1], context_text)
            node_prompt = wrap_llama_chat(node_body, LLAMA_SYSTEM_PROPOSER)

            t0 = time.perf_counter()
            candidates, structured_ct, fallback_ct = _sample_candidates(node_prompt, NODE_SAMPLES)
            record_timing("dvts_node_sample", time.perf_counter() - t0)

            total_samples += NODE_SAMPLES
            total_parsed += len(candidates)
            total_structured += structured_ct
            total_fallback += fallback_ct

            if not candidates:
                # If no valid candidates, force "no" at this node.
                next_id = trans.get("no", "FINAL:no")
                expanded.append({
                    "node_id": next_id,
                    "score": s.get("score", 0.0) + math.log(1e-6),
                    "trace": s.get("trace", []) + [{"node_id": node_id, "option": "no", "rationale": "No valid candidate"}],
                })
                continue

            yes_ct = sum(1 for c in candidates if str(c.get("option", "")).lower() == "yes")
            no_ct = sum(1 for c in candidates if str(c.get("option", "")).lower() == "no")
            denom = max(1, yes_ct + no_ct)

            # Use verifier to pick up to `dvts_branch` candidate steps for this state.
            pick_idx = _verifier_pick_indices(node_prompt, candidates, dvts_branch)
            if not pick_idx:
                pick_idx = [0]

            for idx in pick_idx:
                c = candidates[idx]
                opt = str(c.get("option", "")).lower()
                if opt not in ("yes", "no"):
                    opt = "no"
                prob = (yes_ct / denom) if opt == "yes" else (no_ct / denom)
                prob = max(prob, 1e-6)
                next_id = trans.get(opt, "FINAL:no")
                expanded.append({
                    "node_id": next_id,
                    "score": s.get("score", 0.0) + math.log(prob),
                    "trace": s.get("trace", []) + [{"node_id": node_id, "option": opt, "rationale": str(c.get("rationale", ""))}],
                })

        # Keep at most dvts_trees active states with diversity control.
        states = _prune_states(expanded, max_states=dvts_trees, per_key=dvts_per_key, key_depth=dvts_key_depth)

    # Choose the best terminal state (prefer FINAL:yes if any exist).
    yes_states = [s for s in states if str(s.get("node_id", "")).strip().lower() == "final:yes"]
    if yes_states:
        best = max(yes_states, key=lambda x: x.get("score", float("-inf")))
        final_opt = "yes"
    else:
        best = max(states, key=lambda x: x.get("score", float("-inf"))) if states else {"node_id": "FINAL:no", "score": 0.0, "trace": []}
        final_opt = "yes" if str(best.get("node_id", "")).strip().lower() == "final:yes" else "no"

    trace_lines = []
    for step in best.get("trace", []):
        nid = step.get("node_id", "")
        opt = step.get("option", "")
        rat = step.get("rationale", "")
        trace_lines.append(f"{nid}={opt}; Evidence=\"{rat}\"")
    rationale = " | ".join(trace_lines) if trace_lines else "No steps"

    votes = {
        "rule": "dvts",
        "trees": dvts_trees,
        "branch": dvts_branch,
        "key_depth": dvts_key_depth,
        "per_key": dvts_per_key,
        "samples": total_samples,
        "parsed": total_parsed,
        "json_structured_samples": total_structured,
        "json_fallback_samples": total_fallback,
    }
    return {"rationale": rationale, "option": final_opt, "votes": votes}
...
def process_batches(notes, question_dictionary, batch_size=50):
    ...
    for question_key, question_info in question_dictionary.items():
        ...
        if splits:
            ...
            response = run_dvts(question_key, context_text, atomic_tree)
        ...
        if file.endswith(".json"):
            ...
            with open(file_path, 'r') as f:
                try:
                    ...
                    logger.warning(f"Error decoding JSON in file: {file}")
                    continue
            yield file_name, file_content

directory_path = "/home/ec2-user/fhir/mrn_json_files/"

output_dir = 'complication_results_folder/complication_results_wt_test5_llama_DVTS_a'
os.makedirs(output_dir, exist_ok=True)
VOTE_LOG_PATH = os.path.join(output_dir, os.environ.get('VOTE_LOG_FILENAME', 'vote_log.csv'))
fhir_data = pd.read_json("./features_folder/vap_only_patient_features.json") # temporary
...
for idx, bundle in enumerate(fhir_data, start=1):
    ...
    if os.path.exists(os.path.join(output_dir, f'{csn}.json')):
        logger.info(f"[{idx}/{total_features}] Data for {csn} already exists. Skipping...")
