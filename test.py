...
output_dir = 'complication_results_wt_FINAL/'
aggregated_results = aggregate_results(output_dir)
df = pd.read_csv('/home/ec2-user/fhir/ground_truth.csv', dtype={'csn':str})
...
for v in COMPLICATIONS:
    print("=========================")
    print(v)
    print("Sensitivity: " + str(TP[v]) + "/" + str(TP[v] + FN[v]) + " (" + str(TP[v] / (TP[v] + FN[v])) + ")")
    print("NPV: "  + str(TN[v]) + "/" + str(TN[v] + FN[v]) + " (" + str(TN[v] / (TN[v] + FN[v])) + ")")
    print("PPV: " +str(TP[v]) + "/" + str(TP[v] + FP[v]) + " (" +str(TP[v] / (TP[v] + FP[v])) + ")")
    print("=========================")
    TP_total += TP[v]
    TN_total += TN[v]
    FP_total += FP[v]
